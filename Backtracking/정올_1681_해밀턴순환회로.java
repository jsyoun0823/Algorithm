...
public class 정올_1681_해밀턴순환회로 {
	...
	private static int min, sum; // 회사에서 출발하여 오늘 배달해야 하는 모든 장소를 한 번씩 들러 물건을 배달하고 회사에 돌아오기 위한 최소의 비용
	
	public static void main(String[] args) throws NumberFormatException, IOException {
		BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
		N = Integer.parseInt(br.readLine().trim()); // 장소의 수 N (N(1≤N≤12)
		cost = new int[N][N]; // 장소와 장소를 이동하는 비용이 저장될 배열 
		for (int i = 0; i < N; i++) {
	...
	public static void dfs(int r, int c, int cnt) {
		...
		if(cnt == N-1) { // 다 구했으면
			sum += cost[c][0]; // 회사로 다시 돌아가는 경우의 비용을 더해주기
			if(cost[c][0] == 0) return; // 이때 회사로 돌아가는 길이 이어져 있지 않으면 리턴
			min = (min > sum) ? sum : min; // 최소값 갱신
			sum -= cost[c][0]; // 다음 계산을 위해 회사로 돌아가는 비용 원복해주기
			return;
		}
